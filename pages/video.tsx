      ...
      <section className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8 lg:py-20">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-12">
          {mediaTopics.map((topic, index) => <Link key={topic.title} href={topic.href} className={`group overflow-hidden border border-[var(--industrial-line)] bg-[var(--industrial-surface)] ${index < 2 ? 'lg:col-span-6' : 'lg:col-span-3'}`}>
            <div className={`relative overflow-hidden ${index < 2 ? 'aspect-[16/9]' : 'aspect-[4/3]'}`}><SiteImage src={topic.image} alt={`${topic.title} product cover`} fill sizes={index < 2 ? '(min-width: 1024px) 50vw, 100vw' : '(min-width: 1024px) 25vw, 50vw'} className="object-cover transition-transform duration-500 group-hover:scale-[1.025]" /></div>
