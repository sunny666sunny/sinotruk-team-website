...
import { parseEditorialContent } from '@/lib/content/editorial-content'
...
export default function NewsDetailPage({ item, previous, next, related, relatedProducts }: NewsDetailProps) {
  ...
  const blocks = parseEditorialContent(item.content)

  return <div className="industrial-page">
    <SeoHead input={{ path: `/news/${item.slug}`, pageType: 'article', name: item.seoTitle, description: item.seoDescription, image: item.image, datePublished: item.date, dateModified: item.updatedAt, override: { keywords: item.keywords.join(', ') }, source: { title: item.sourceTitle, url: item.sourceUrl, datePublished: item.sourceDate }, breadcrumbs: [{ name: 'Home', path: '/' }, { name: 'News', path: '/news' }, { name: item.title, path: `/news/${item.slug}` }] }} />
    <Header />
    <main id="main" className="pt-16 lg:pt-[72px]">
      <section className="border-b border-[var(--industrial-line)] bg-[var(--industrial-surface)]">
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:px-8 lg:py-12">
          <div>
            ...
            <div className="mt-8 flex flex-wrap items-center gap-4 text-xs uppercase tracking-[.1em] text-[var(--industrial-accent)]"><span>{getNewsCategory(item)}</span><time dateTime={item.date} className="inline-flex items-center gap-2 text-[var(--industrial-muted)]"><Calendar className="h-4 w-4" />Published {formatDate(item.date)}</time>{item.updatedAt && <time dateTime={item.updatedAt} className="text-[var(--industrial-muted)]">Reviewed {formatDate(item.updatedAt)}</time>}</div>
      ...
      <article className="py-12 lg:py-16">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[minmax(0,45rem)_minmax(17rem,1fr)] lg:px-8">
          <div className="text-[1.05rem] leading-8 text-[var(--industrial-text)]">{blocks.map((block, index) => block.type === 'heading'
            ? <h2 key={index} className="mb-5 mt-10 [font-family:var(--industrial-display)] text-3xl font-semibold uppercase leading-tight first:mt-0">{block.text}</h2>
            : block.type === 'list'
              ? <ul key={index} className="my-6 space-y-3 border-l-2 border-[var(--industrial-accent)] pl-6">{block.items.map((value) => <li key={value} className="relative before:absolute before:-left-5 before:text-[var(--industrial-accent)] before:content-['—']">{value}</li>)}</ul>
              : <p key={index} className="mb-6">{block.text}</p>)}</div>
          <aside className="h-fit border-t border-[var(--industrial-accent)] bg-[var(--industrial-surface)] p-6 text-sm leading-6 text-[var(--industrial-muted)] lg:sticky lg:top-28">
            ...
            <p className="mt-3">Reviewed by <strong className="text-[var(--industrial-text)]">SINOTRUK TEAM Editorial</strong></p>
        ...
        <div className="mx-auto mt-12 max-w-7xl px-4 sm:px-6 lg:px-8"><section aria-labelledby="article-next-steps" className="border-t border-[var(--industrial-line)] pt-8"><h2 id="article-next-steps" className="[font-family:var(--industrial-display)] text-3xl font-semibold uppercase">Continue with verified catalogue information</h2><div className="mt-5 flex flex-wrap gap-3">{item.internalLinks.map((href) => <Link key={href} href={href} className="inline-flex min-h-11 items-center gap-2 border border-[var(--industrial-line)] px-4 font-semibold hover:border-[var(--industrial-accent)]">{href === '/contact' ? 'Prepare an enquiry' : href === '/parts' ? 'Browse spare parts' : href === '/service' ? 'Review service information' : 'Browse relevant products'} <ArrowRight className="h-4 w-4" /></Link>)}</div></section></div>

        <div className="mx-auto mt-14 grid max-w-7xl gap-4 border-t border-[var(--industrial-line)] px-4 pt-8 sm:px-6 md:grid-cols-2 lg:px-8">{previous ? <Link href={`/news/${previous.slug}`} className="border border-[var(--industrial-line)] p-5 hover:border-[var(--industrial-accent)]"><span className="inline-flex items-center gap-2 text-xs uppercase tracking-[.1em] text-[var(--industrial-muted)]"><ArrowLeft className="h-4 w-4" />Previous</span><span className="mt-2 block font-semibold">{previous.title}</span></Link> : <div />}{next ? <Link href={`/news/${next.slug}`} className="border border-[var(--industrial-line)] p-5 text-right hover:border-[var(--industrial-accent)]"><span className="inline-flex items-center gap-2 text-xs uppercase tracking-[.1em] text-[var(--industrial-muted)]">Next<ArrowRight className="h-4 w-4" /></span><span className="mt-2 block font-semibold">{next.title}</span></Link> : <div />}</div>
      </article>
