...
export const getServerSideProps: GetServerSideProps = async ({ res }) => {
  const [products, parts, news] = await Promise.all([
    prisma.product.findMany({ where: { isActive: true }, select: { id: true, categoryId: true, subcategoryId: true, updatedAt: true } }),
  ...
  const entries = [
    ...
    ...products.map((item) => ({ url: productPublicPath(item.categoryId, item.subcategoryId, item.id), lastModified: item.updatedAt })),
